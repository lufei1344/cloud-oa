		...
		<div class="row">
                <div class="col-sm-6">
                    <div class="input-group">
                        <input placeholder="菜单名称" type="text" class="input-sm form-control" name="filter_LIKES_name" value="${param['filter_LIKES_name']}"/> 
				  ...
				  <div class="input-group">
                       <span class="input-group-btn">
                           <c:choose>
							...
							<a type='button' href="${ctx}/security/menu/create" class="btn btn-sm btn-primary" >新建</a>
							</shiro:hasPermission>
							</c:when>
						</c:choose>
                       </span>
			...
			<c:forEach items="${page.result}" var="menu">
				<tr>
				    ...
				    <shiro:hasPermission name="MENUDELETE">
						<a href="${ctx}/security/menu/delete/${menu.id }" class="glyphicon glyphicon-trash" title="删除" onclick="return confirmDel();">删除</a>
					</shiro:hasPermission>
					<shiro:hasPermission name="MENUEDIT">
						<a href="${ctx}/security/menu/update/${menu.id }" class="glyphicon glyphicon-pencil" title="编辑">编辑</a>
					</shiro:hasPermission>
						<a href="${ctx}/security/menu/view/${menu.id }" class="glyphicon glyphicon-search" title="查看">查看</a>
					</c:when>
					<c:otherwise>
						<a href="javascript:void(0)" class="glyphicon glyphicon-search" title="选择" onclick="parent.digCallBack({id:'${menu.id}',name:'${menu.name }'})">选择</a>
					</c:otherwise>
					</c:choose>
					</td>
				</tr>
