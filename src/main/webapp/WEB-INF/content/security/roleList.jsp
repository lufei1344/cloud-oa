		...
		<div class="row">
                <div class="col-sm-6">
                    <div class="input-group">
                        <input placeholder="角色名称" type="text" class="input-sm form-control" name="filter_LIKES_name" value="${param['filter_LIKES_name']}"/> 
				  ...
				  <div class="input-group">
                       <span class="input-group-btn">
                           <c:choose>
							...
							<a type='button' href="${ctx}/security/role/create" class="btn btn-sm btn-primary" >新建</a>
							</shiro:hasPermission>
							</c:when>
							<c:otherwise>
			...
			<c:forEach items="${page.result}" var="role">
				<tr>
				    ...
				    <shiro:hasPermission name="ROLEDELETE">
						<a href="${ctx}/security/role/delete/${role.id }"  class="glyphicon glyphicon-trash" title="删除" onclick="return confirmDel();">删除</a>
					</shiro:hasPermission>
					<shiro:hasPermission name="ROLEEDIT">
						<a href="${ctx}/security/role/update/${role.id }" class="glyphicon glyphicon-pencil"  title="编辑">编辑</a>
					</shiro:hasPermission>
						<a href="${ctx}/security/role/view/${role.id }" class="glyphicon glyphicon-search"  title="查看">查看</a>
					</c:when>
					<c:otherwise>
						<a href="javascript:void(0)" class="glyphicon glyphicon-search"  title="选择" onclick="parent.digCallBack({id:'${role.id}',name:'${role.name }'})">选择</a>
					</c:otherwise>
					</c:choose>
					</td>
				</tr>
