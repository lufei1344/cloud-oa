...
<head>
	...
	<body>
	<form id="mainForm" action="${ctx}/form/list" method="get">
				  ...
				  <div class="input-group">
                       <span class="input-group-btn">
                           <c:choose>
							...
							<a type='button' href="${ctx}/form/create" class="btn btn-sm btn-primary" >新建</a>
							</shiro:hasPermission>
							</c:when>
						</c:choose>
                       </span>
          ...
          <table class="table table-striped table-hover table-bordered">
			...
			<c:forEach items="${page.result}" var="form">
				<tr>
					...
					<c:choose>
                        <c:when test="${empty lookup}">
						<a href="${ctx}/form/delete/${form.id }" class="glyphicon glyphicon-trash" title="删除" onclick="return confirmDel();">删除</a>
						<a href="javascript:void(0)" onclick="editDialog('${ctx}/form/update/${form.id }')" class="glyphicon glyphicon-pencil" title="编辑">编辑</a>
						<a href="${ctx}/form/view/${form.id }" class="glyphicon glyphicon-search" title="查看">查看</a>
						</c:when>
                        <c:otherwise>
